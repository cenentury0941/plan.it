...
import { setupItineraryListener } from '@/lib/firebase'
...
export default function Plan(){
    ...
    const [ Items, setItinerary ] = useState<Place[]>([])
    ...
    useEffect( () => {
        ...
        setupItineraryListener(getOrCreateChatUUID(), setItinerary)
    } , [] )

    useEffect( () => {
        dispatch(setPlaces(Items))
    } , [Items] )
    ...
    return <div className="pointer-events-auto absolute left-0 h-full w-[28%] pr-0.75 bg-linear-to-br to-[#9d78ff] from-[#f79ec8] shadow-[10px_5px_20px_rgba(14,5,23,0.39)] reveal-left">
        <div ref={contentRef} className="h-full w-full bg-black/60 pt-[2%]">
            <div className='relative flex px-[5%] flex-row h-[calc(100%-93vh)] w-full place-content-center place-items-center gap-2'>
                ...
                <div className="h-[3.9vh] w-[3.9vh] bg-transparent transition duration-150 hover:bg-linear-to-bl hover:from-pink-300 hover:to-purple-400 rounded-full p-0.5 cursor-pointer"><div onClick={reactToPrintFn} className="h-full w-full p-1 transition duration-150 hover:bg-black/80 rounded-full"><FileDown className='h-full w-full' color='#dac4ff'/></div></div>
            ...
            <div className="relative h-[calc(100%-6vh)] grow-0">
                ...
                {Items.map( (item) => {return <PlanItem key={item.id} place={item} selected={flyToPlace && (item.id==flyToPlace.id)}/>} )}
